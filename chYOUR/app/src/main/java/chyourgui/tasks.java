...
public class tasks extends AppCompatActivity implements View.OnClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        for (Integer key : taskMap.keySet()) {
            ...
            temp += "Start time: " + taskMap.get(key).get(4)
                    + "/" + taskMap.get(key).get(5) + " "
                    + taskMap.get(key).get(6) + ":" + taskMap.get(key).get(7)
                    + taskMap.get(key).get(8) + '\n';

            temp += "End time: " + taskMap.get(key).get(9)
                    + "/" + taskMap.get(key).get(10) + " "
                    + taskMap.get(key).get(11) + ":" + taskMap.get(key).get(12)
                    + taskMap.get(key).get(13);
            bt.setText(temp);
            layout2.addView(bt);
            bt.setId(key);
